...
<!-- Title: Contributing GitFlow EmailSignature -->
...
To contribute to email-signature project on [GitHub](https://github.com/luismayta/email-signature), We use Gitflow (See more [here](https://datasift.github.io/gitflow/IntroducingGitFlow.html)) In a nutshell, it means that you should branch from the main repository and contribute back by making [pull request](https://github.com/luismayta/email-signature/pulls).

![workflow gitflow](https://datasift.github.io/gitflow/GitFlowHotfixBranch.png)

## Getting started

To follow the instructions in this guide and start contributing to email-signature project on Gitlab:
...
```bash
git remote add upstream  https://github.com/luismayta/email-signature.git
```

Now your **upstream** points to **luismayta/email-signature**.
...
git fetch upstream # assuming upstream is luismayta/email-signature
...
All merge requests are automatically tested using [Github Actions](https://github.com/luismayta/email-signature/actions). In case some tests fail, fix the issues or describe why the fix cannot be done.
